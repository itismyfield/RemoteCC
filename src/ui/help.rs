...
fn build_help_content(theme: &Theme, kb: &Keybindings) -> Vec<Line<'static>> {
    ...
    lines.push(Line::from(vec![
        Span::styled("  ".to_string(), desc_style),
        Span::styled("Config: ~/.remotecc/settings.json".to_string(), hint_style),
    ]));
    lines.push(Line::from(vec![
        Span::styled("  ".to_string(), desc_style),
        Span::styled("Themes: ~/.remotecc/themes/".to_string(), hint_style),
    ]));
    lines.push(Line::from(""));
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            "https://github.com/itismyfield/RemoteCC".to_string(),
            desc_style,
        ),
    ]));
    lines.push(Line::from(vec![
